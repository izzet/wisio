...
char* get_filename(Record* record) {
    ...
    if(read_condition && !fread_condition && !readdir_condition && !readlink_condition) return record->args[0];
    if(fread_condition) return get_record_arg(func_name, record, 3);
    ...
    if(vfprintf_condition) return record->args[0];
    if(remove_condition) return record->args[0];
    if(unlink_condition) return record->args[0];
    if(access_condition) return record->args[0];
...
int64_t get_size(Record* record) {
    ...
    const char* lseek_condition = strstr(func_name, "lseek");
    const char* fseek_condition = strstr(func_name, "fseek");
    if(read_condition && !fread_condition && !readdir_condition && !readlink_condition) return atol(record->args[2]);
    ...
    if(lseek_condition || fseek_condition) return atol(record->args[1]);
