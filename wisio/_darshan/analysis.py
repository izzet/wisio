...
def create_dxt_dataframe(trace_path: str, time_granularity=1e3):
    ...
    for r in zip(
        dxt_posix['rank'],
        dxt_posix['read_segments'],
        dxt_posix['write_segments'],
        dxt_posix['id'],
        dxt_posix['hostname'],
    ):
        if not r[1].empty:
            ...
            read_io_cat.append(
                [IOCategory.READ.value] * len((r[1]['length'].to_list()))
            )
        ...
        if not r[2].empty:
            ...
            write_io_cat.append(
                [IOCategory.WRITE.value] * len((r[2]['length'].to_list()))
            )

    file_name = [
        rep.data['name_records'][element]
        for nestedlist in read_id
        for element in nestedlist
    ]
    rank = [
        f"app#localhost#{element}#0"
        for nestedlist in read_rank
        for element in nestedlist
    ]
    ...
    time_range = [
        int(element * time_granularity)
        for nestedlist in read_start_time
        for element in nestedlist
    ]
    ...
    time_range.extend(
        [
            int(element * time_granularity)
            for nestedlist in write_start_time
            for element in nestedlist
        ]
    )
    hostname.extend(
        [element for nestedlist in write_hostname for element in nestedlist]
    )
